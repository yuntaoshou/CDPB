        ...
        for epoch in range(args.epochs):
            ...
            with torch.no_grad():
                ...
                if val_best_score < c_index:
                    ...
                    torch.save(model.state_dict(), os.path.join(current_path, f'pretraining/first_{args.source_dataset}.pth'))
...
def EM_training(args):
    ...
    for fold in range(5):
        ...
        model_first.load_state_dict(torch.load(f'pretraining/first_{args.source_dataset}.pth', map_location=f'cuda:{args.device}'))
        ...
        model_second = PathNN(args, args.num_features, args.projection_size, args.cutoff, args.prediction_size, args.dropout, args.device, residuals=True, 
                       encode_distances=False, perturb_position=args.pp)
        # model_second.load_state_dict(torch.load(f'pretraining/second_{args.source_dataset}.pth', map_location=f'cuda:{args.device}'))
        ...
        for em_step in range(args.EM_epochs):
            if os.path.exists(f'./pretraining/M_second_{args.source_dataset}.pth'):
            ...
            for i in range(1):
                ...
                if total_best_train_cindex < E_target_cindex:
                    ...
                    torch.save(model_first.state_dict(),
                            os.path.join(current_path, f'./pretraining/E_first_{args.source_dataset}_E.pth'))
            ...
            for i in range(1):
                ...
                if total_best_train_cindex < M_target_cindex:
                    ...
                    torch.save(model_second.state_dict(),
                            os.path.join(current_path, f'./pretraining/M_second_{args.source_dataset}_M.pth'))
